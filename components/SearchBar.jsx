...
const SearchBar = () => {
    ...
    return(        
        <form type="submit" onSubmit={handleFormSubmit}>
            <div className={"rounded-2xl bg-stone-700 mb-10 md:mb-4 md:mb-8 md:pb-3 pb-5 md:pt-2 pt-3.5 2xl:py-4 pl-0.5 md:pl-4 lg:pl-4 md:gap-x-0 xl:gap-x-8 grid sm:grid-rows-3 md:grid-rows-2 lg:grid-rows-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-4 md:gap-y-3 lg:gap-y-4 w-72 md:w-[34.5rem] lg:w-[75.5%] xl:w-[59.5%] sm:content-center"}>
                <div className={"bg-stone-300 rounded-l-lg mt-1 w-[20%] md:w-[27.25%] lg:w-4/12 2xl:w-[20%] md:rounded-r-2xl-0 h-[3.25rem] md:h-14 2xl:h-16 ml-6 md:mx-2 lg:ml-2"}>
                    <div className={"relative inline-block gap-x-3 justify-start content-center w-60 md:w-[15rem] lg:w-[13.35rem] xl:w-60 2xl:w-80 -mb-0.5 md:mb-0"}>
                        ...
                        <Combobox value={selectPassport} onChange={(passport) => setSelectedPassport(passport)}>
                            <Combobox.Input ref={inputRef} onChange={(event) => setPassportQuery(event.target.value)} placeholder={INITIAL_PASSPORT_STATE} className={"float-right h-14 2xl:h-16 2xl:w-[79%] p-2 py-4 text-stone-600 dark:text-stone-200 text-[1.025rem] border-transparent rounded-r-xl font-medium"}/>
                ...
                <div className={"bg-stone-300 rounded-l-lg mt-1 w-[23%] md:w-[27.25%] lg:w-4/12 2xl:w-[20%] h-[3.25rem] md:h-14 2xl:h-16 ml-6 md:mx-6 md:ml-2 lg:ml-2"}>
                    <div className={"relative inline-block justify-start content-center w-60 lg:w-[15rem] xl:w-60 2xl:w-80"}>      
                        ...
                        <Combobox value={selectDestination} onChange={(destination) => setSelectedDestination(destination)}>
                            <Combobox.Input ref={inputRef} onChange={(event) => setDestinationQuery(event.target.value)} placeholder={INITIAL_DESTINATION_STATE} className={"float-right h-14 2xl:h-16 w-9/12 2xl:w-[79%] p-2 py-4 text-stone-600 dark:text-stone-200 text-[1.025rem] border-transparent rounded-r-xl font-medium"} />
                                <div className="inline-block w-full">
                                    <Combobox.Options className={"z-40 absolute mt-2 ml-14 pb-0.5 max-h-80 shadow-2xl bg-stone-100 overflow-y-auto rounded-xl text-sm text-stone-600 justify-start list-none w-3/4 scrollbar"}>
